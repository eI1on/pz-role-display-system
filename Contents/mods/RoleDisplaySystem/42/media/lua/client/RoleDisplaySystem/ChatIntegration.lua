...
RoleDisplaySystem.ChatIntegration = RoleDisplaySystem.ChatIntegration or {}
RoleDisplaySystem.ChatIntegration.isInitialized = RoleDisplaySystem.ChatIntegration.isInitialized or false
...
function RoleDisplaySystem.ChatIntegration.FormatPlayerRoles(username, forTextDraw)
	...
	local coloredRoles = {}
	local rolesToShow = RoleDisplaySystem.Shared.GetRolesForDisplay(roles, RoleDisplaySystem.CHAT_CONFIG.SHOW_MULTIPLE_ROLES)

	for _, role in ipairs(rolesToShow) do
...
function RoleDisplaySystem.ChatIntegration._metaMethodOverwrite.apply(class, methodName)
	...
	if metatable__index["__RoleDisplaySystem_" .. methodName] then
		return true
	end
	metatable__index["__RoleDisplaySystem_" .. methodName] = originalMethod
...
function RoleDisplaySystem.ChatIntegration.Initialize()
	if RoleDisplaySystem.ChatIntegration.isInitialized then
		return
	end
	...
	if success then
		RoleDisplaySystem.ChatIntegration.isInitialized = true
