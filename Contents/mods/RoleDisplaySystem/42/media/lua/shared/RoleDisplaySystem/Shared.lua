...
function RoleDisplaySystem.Shared.RequestRoles()
	if isClient() and not (isServer and isServer()) then
		if RoleDisplaySystem.Client and RoleDisplaySystem.Client.RequestRoles then
			RoleDisplaySystem.Client.RequestRoles(false)
		end
		return RoleDisplaySystem.Roles
	elseif RoleDisplaySystem.Store and RoleDisplaySystem.Store.LoadRoles then
		return RoleDisplaySystem.Store.LoadRoles()
	elseif RoleDisplaySystem.Server and RoleDisplaySystem.Server.LoadRoles then
		return RoleDisplaySystem.Server.LoadRoles()
	else
		return RoleDisplaySystem.Roles or {}
	end
end
...
function RoleDisplaySystem.Shared.ShouldShowRoleInMultiple(role)
	return not role or role.showInMultiple ~= false
end

function RoleDisplaySystem.Shared.GetRolesForDisplay(roles, showMultiple)
	local displayRoles = {}
	...
	if showMultiple and #roles > 1 then
		for _, role in ipairs(roles) do
			if RoleDisplaySystem.Shared.ShouldShowRoleInMultiple(role) then
				displayRoles[#displayRoles + 1] = role
			end
		end
	elseif roles[1] then
		displayRoles[1] = roles[1]
	end
